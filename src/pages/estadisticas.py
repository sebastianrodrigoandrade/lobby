...
def render():
    ...
    tabs = st.tabs(["📜 Leyes y Proyectos", "⚡ DNUs", "📊 Fuentes"])
    ...
    with tabs[0]:
        ...
        if df_leyes.empty:
            ...
            col1.metric("Total leyes", f"{total_leyes:,}")
    ...
    with tabs[1]:
        ...
        if df_dnus.empty:
            ...
            col1, col2, col3 = st.columns(3)
            col1.metric("Total DNUs", total_dnus)
            col2.metric("Último año", año_max)
            col3.metric("Año récord", f"{año_mas_dnus} ({dnus_max})")

            st.caption("⚠️ Datos 2008–2023. Los DNUs 2024-2025 no están en datos abiertos.")
            ...
            busqueda_dnu = st.text_input("🔍 Buscar", placeholder="Ej: jubilaciones, emergencia...", key="dnu_busq")
            
            df_mostrar = df_dnus_det.copy()
            if busqueda_dnu:
                df_mostrar = df_mostrar[df_mostrar['titulo'].str.contains(busqueda_dnu, case=False, na=False)]

            st.markdown(f"**{len(df_mostrar)} DNUs**")
            
            st.dataframe(
                df_mostrar[['fecha_ingreso', 'nro_expediente', 'titulo']].rename(columns={
                    'fecha_ingreso': 'Fecha', 'nro_expediente': 'Expediente', 'titulo': 'Descripción',
                }),
    ...
    with tabs[2]:
        ...
        <div class="lobby-card">
            ...
            <div class="lobby-card-meta">Votaciones, proyectos, sesiones</div>
        ...
        <div class="lobby-card">
            ...
            <div class="lobby-card-meta">Declaraciones juradas patrimoniales</div>
            <div style="margin-top: 0.5rem;">
                <a href="https://datos.jus.gob.ar/" target="_blank">datos.jus.gob.ar</a>
            </div>
        </div>
        """, unsafe_allow_html=True)
